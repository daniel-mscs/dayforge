      ...
      {modalResumo && (
              <div className="modal-overlay">
                <div className="modal-resumo" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
                  ...
                  {/* Gráfico de cargas */}
                  {(() => {
                    const filtrados = exercicios.filter(ex => ex.treino === (modalResumo.treinoIniciado || treinoAtivo))
                    if (filtrados.length === 0) return null
                    const dadosGrafico = filtrados.map(ex => ({
                      name: ex.nome.length > 10 ? ex.nome.substring(0, 10) + '…' : ex.nome,
                      volume: Number(ex.series || 0) * Number(ex.repeticoes || 0) * Number(ex.carga || 0),
                      carga: Number(ex.carga || 0),
                      concluido: !!concluidos[ex.id],
                    }))
                    return (
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 10, color: '#64748b', fontWeight: 800, letterSpacing: '0.08em', marginBottom: 10 }}>VOLUME POR EXERCÍCIO (séries × reps × kg)</div>
                  ...
                  {/* Lista de exercícios */}
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 10, color: '#64748b', fontWeight: 800, letterSpacing: '0.08em', marginBottom: 10 }}>EXERCÍCIOS DO TREINO</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {exercicios.filter(ex => ex.treino === (modalResumo.treinoIniciado || treinoAtivo)).map(ex => (
                        <div key={ex.id} style={{
                          background: '#24282d', borderRadius: 8, padding: '8px 12px',
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 10, color: '#64748b' }}>{ex.series}x{ex.repeticoes} · {ex.carga}kg</div>
                          </div>
                  ...
                  <div className="modal-actions">
                    ...
                    <button className="btn-cancel-rest" onClick={() => {
                      setModalResumo(null)
            ...
            {!treinando && exerciciosFiltrados.length > 0 && (
                          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
                            <button onClick={async () => {
                              if (!confirm(`Apagar todos os exercícios do Treino ${treinoAtivo}?`)) return
                              await Promise.all(exerciciosFiltrados.map(ex => supabase.from('exercicio').delete().eq('id', ex.id).eq('user_id', user.id)))
                              buscarExercicios()
                            }} style={{
                              background: 'transparent', border: '1px solid #ef444433', borderRadius: 8,
                              color: '#ef4444', fontSize: 12, padding: '5px 12px', cursor: 'pointer'
                            }}>🗑️ Apagar Treino {treinoAtivo}</button>
                          </div>
                        )}
