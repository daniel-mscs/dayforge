...
export default function Rotina({ user }) {
  ...
  const gerarDias = async () => {
    ...
    if (dias.length > 0) {
      if (
        !confirm("Você já tem uma rotina! Deseja apagar tudo e criar uma nova?")
      )
        return;
  ...
  const confirmarClone = async (diaDestinoId) => {
    ...
    if (tarefasOrigem.length === 0) {
      alert("Nenhuma tarefa para clonar!");
      setClonando(false);
      return;
    }
              ...
              return semanasGrid.map((semana, si) => (
                <div key={si} className="rotina-grade-semana">
                  {semana.map((cell, ci) => {
                    ...
                    const totalT = PERIODOS.flatMap(
                      (p) => tarefas[dia.id]?.[p] || [],
                    ).length;
                    const concT = PERIODOS.flatMap(
                      (p) => tarefas[dia.id]?.[p] || [],
